...
def interpretatieve_synthese(analysis: dict[str, Any]) -> None:
    ...
    with st.expander("Verbindingspunten", expanded=False):
        ...
        brugverhalen: list = verbindingspunten.get("brugverhalen", [])
        if brugverhalen:
            st.markdown("**Brugverhalen:**")
    ...
    # ── Specifiek voor datum ───────────────────────────────────────────────────
    with st.expander("Specifiek voor deze datum", expanded=False):
        seizoen: list = specifiek.get("seizoens_associaties", [])
        if seizoen:
            st.markdown("**Seizoensassociaties:**")
            _render_list(seizoen)
        if specifiek.get("verwachting_hoorders"):
            st.markdown(f"**Verwachting hoorders:** {clean_md(specifiek['verwachting_hoorders'])}")
        actualiteit: list = specifiek.get("actualiteit", [])
        if actualiteit:
            st.markdown("**Actualiteit:**")
            _render_list(actualiteit)

    st.divider()

    # ── Homiletische aanbevelingen ────────────────────────────────────────────
    ...
    balans: dict = aanbevelingen.get("balans", {})
    if balans:
        c1, c2 = st.columns(2)
        with c1:
            if balans.get("pastoraal_profetisch"):
                st.info(f"**Pastoraal–profetisch:** {clean_md(balans['pastoraal_profetisch'])}")
        with c2:
            if balans.get("troost_vermaning"):
                st.info(f"**Troost–vermaning:** {clean_md(balans['troost_vermaning'])}")

    waarschuwingen: list = aanbevelingen.get("waarschuwingen", [])
    if waarschuwingen:
        st.markdown("**Waarschuwingen**")
        for w in waarschuwingen:
            with st.container(border=True):
                st.warning(f"**{w.get('onderwerp', '')}** — {clean_md(w.get('risico', ''))}")
                if w.get("advies"):
                    st.markdown(f"*Advies:* {clean_md(w['advies'])}")
