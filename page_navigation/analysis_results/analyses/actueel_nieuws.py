...
def actueel_nieuws(analysis: dict[str, Any]) -> None:
            ...
            with st.expander(f"{icon} {item.get('titel', '')}  — *{cat}*", expanded=False):
                ...
                with c2:
                    ...
                    if item.get("emotionele_impact"):
                        st.caption(f"Impact: {item['emotionele_impact']}")
                ...
                bronnen: list = item.get("bronnen_urls", [])
                if bronnen:
                    st.caption("Bronnen: " + " · ".join(bronnen))
                ...
                with st.container(border=True):
                    ...
                    with c1:
                        if item.get("lokale_relevantie"):
                            st.caption(f"Lokale relevantie: {clean_md(item['lokale_relevantie'])}")
                    with c2:
                        if item.get("relevantie_preek"):
                            st.caption(f"Preek: {clean_md(item['relevantie_preek'])}")
                ...
                with st.container(border=True):
                    ...
                    with c1:
                        if item.get("relevantie_oecumenisch"):
                            st.caption(f"Oecumenisch: {clean_md(item['relevantie_oecumenisch'])}")
                    with c2:
                        if item.get("relevantie_preek"):
                            st.caption(f"Preek: {clean_md(item['relevantie_preek'])}")
                    bronnen: list = item.get("bronnen_urls", [])
    ...
    # ── Suggesties predikant ──────────────────────────────────────────────────
    st.subheader("Suggesties voor de predikant")
    c1, c2, c3 = st.columns(3)
    with c1:
        st.markdown("**In de preek**")
        in_preek: list = suggesties.get("in_de_preek", [])
        _render_list(in_preek)
    with c2:
        st.markdown("**In de voorbeden**")
        in_voorbeden: list = suggesties.get("in_voorbeden", [])
        _render_list(in_voorbeden)
    with c3:
        st.markdown("**Mededelingen / collecte**")
        mededeling: list = suggesties.get("mededelingen_collecte", [])
        _render_list(mededeling)
